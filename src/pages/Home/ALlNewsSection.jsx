...
const ALlNewsSection = () => {
  ...
  return (
    <>
      <div className='grid grid-cols-3 gap-4 sm:grid-cols-2 m-auto'>
        {
          newData.map((item) => {
            ...
            return (
              ...
              >
                <img src={img} className='h-full w-full hover:scale-110 transition-all cursor-pointer '></img>
                ...
                <p className={`absolute top-8 right-5 md:right-2 xs:right-1 text-white bg-customGreen p-1 rounded-md ${id === 0 ? 'text-base md:text-sm' : 'text-xs md:p-0.5 xs:text-customSize'}`}>
                  {date}
                </p>
                <p
